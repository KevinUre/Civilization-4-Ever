...
##### DeathMaker900 START
			CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_FOREVER	: self.placeForeverConcepts,
##### DeathMaker900 END
...
##### DeathMaker900 START
		self.szCategoryConceptForever = localText.getText("TXT_KEY_PEDIA_CATEGORY_CONCEPT_FOREVER", ())
##### DeathMaker900 END
...
##### DeathMaker900 START
	def placeForeverConcepts(self):
		screen = self.getScreen()
		
		# Create and place a tech pane									
		list = self.getSortedList( gc.getNumForeverConceptInfos(), gc.getForeverConceptInfo )
		...
		for item in list:
			...
			if iRow >= iNumRows:
				iNumRows += 1
				screen.appendTableRow(tableName)
			screen.setTableText(tableName, iColumn, iRow, u"<font=3>" + item[0] + u"</font>", gc.getForeverConceptInfo(item[1]).getButton(), WidgetTypes.WIDGET_PEDIA_DESCRIPTION, CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_FOREVER, item[1], CvUtil.FONT_LEFT_JUSTIFY)
			iCounter += 1
##### DeathMaker900 END
...
##### DeathMaker900 START
		if (szLink == "PEDIA_MAIN_CONCEPT_FOREVER"):
			return self.pediaJump(CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_FOREVER), True)
##### DeathMaker900 END	
