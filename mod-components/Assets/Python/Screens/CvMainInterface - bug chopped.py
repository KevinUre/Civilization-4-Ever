...
# BUG - Options - end
	...
	def updatePercentButtons( self ):
		...
		for iI in range( CommerceTypes.NUM_COMMERCE_TYPES ):
			szString = "IncreasePercentLarge" + str(iI)
			screen.hide( szString )
			szString = "IncreasePercent" + str(iI)
			screen.hide( szString )
			szString = "DecreasePercent" + str(iI)
			screen.hide( szString )
			szString = "DecreasePercentLarge" + str(iI)
			screen.hide( szString )
			#szString = "IncreasePercent" + str(iI)
			#screen.hide( szString )
			#szString = "DecreasePercent" + str(iI)
			...
			if ( CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_ADVANCED_START):
				for iI in range( CommerceTypes.NUM_COMMERCE_TYPES ):
					...
					if (gc.getActivePlayer().isCommerceFlexible(eCommerce) or (CyInterface().isCityScreenUp() and (eCommerce == CommerceTypes.COMMERCE_GOLD))):
						szString1 = "IncreasePercentLarge" + str(eCommerce)
						screen.setButtonGFC( szString1, u"", "", 70, 50 + (19 * iCount), 20, 20, WidgetTypes.WIDGET_CHANGE_PERCENT, eCommerce, gc.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS2"), ButtonStyles.BUTTON_STYLE_CITY_PLUS )
						screen.show( szString1 )
						szString2 = "IncreasePercent" + str(eCommerce)
						screen.setButtonGFC( szString2, u"", "", 90, 50 + (19 * iCount), 20, 20, WidgetTypes.WIDGET_CHANGE_PERCENT, eCommerce, gc.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS"), ButtonStyles.BUTTON_STYLE_CITY_PLUS )
						screen.show( szString2 )
						szString3 = "DecreasePercent" + str(eCommerce)
						screen.setButtonGFC( szString3, u"", "", 110, 50 + (19 * iCount), 20, 20, WidgetTypes.WIDGET_CHANGE_PERCENT, eCommerce, -gc.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS"), ButtonStyles.BUTTON_STYLE_CITY_MINUS )
						screen.show( szString3 )
						szString4 = "DecreasePercentLarge" + str(eCommerce)
						screen.setButtonGFC( szString4, u"", "", 130, 50 + (19 * iCount), 20, 20, WidgetTypes.WIDGET_CHANGE_PERCENT, eCommerce, -gc.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS2"), ButtonStyles.BUTTON_STYLE_CITY_MINUS )
...
# BUG - Great Person Bar - start
		if ( CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY  and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_ADVANCED_START):
			self.updateGreatPersonBar(screen) #FINDME FIND ME
...
# BUG - Great Person Bar - start
				self.updateGreatPersonBar(screen) # FIND ME FINDME
# BUG - Great Person Bar - end

# BUG - Great General Bar - start
				self.updateGreatGeneralBar(screen) # FIND ME FINDME
# BUG - Great General Bar - end
					
		return 0
		
# BUG - Great Person Bar - start
	def updateGreatPersonBar(self, screen): # FIND ME FINDME
 		if (not CyInterface().isCityScreenUp()):
 			pGPCity, iGPTurns = GPUtil.getDisplayCity()
 			szText = GPUtil.getGreatPeopleText(pGPCity, iGPTurns, GP_BAR_WIDTH, False, True, True) # FINDME FIND ME TRUE FALSE TRUE ???
...
# BUG - Great General Bar - start
	def updateGreatGeneralBar(self, screen): # FIND ME FINDME
		screen.show( "GreatGeneralBar-w" )
 		if (not CyInterface().isCityScreenUp()):
